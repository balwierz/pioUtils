...
# usage: $1 - bam file of ATAC seq; [extsize=50] [assembly=danRer7] [binSize=10]
...
if [[ -z $1 ]]
then
        echo "Usage: sbatch thisFILE atac.bam <extSize=50> <assembly=danRer7> <binSize>"
        exit 1
fi
...
bam2wig.pl --temp $thisTemp --extend --rpm --nope --shift --shiftval $((-$extSize/2 + 4)) --extval $extSize --var --bin $binSize --in $1 --cpu 8 --nogz --bw --out $bb.cuts.ext$extSize.binSize$binSize.bw
rmdir $thisTemp
